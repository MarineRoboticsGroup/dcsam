...
TEST(TestSuite, mm_factor_elimination) {
  ...
  // Set up discrete keys
  gtsam::DiscreteKey lm1_class(lc1, 2);
  gtsam::DiscreteKey lm2_class(lc2, 2);
  gtsam::DiscreteKeys dks({lm1_class, lm2_class});

  // Add bearing range measurement
  gtsam::Rot2 bearing1 = gtsam::Rot2::fromDegrees(0.0);
  double range1 = 1.0;
  // Add semantic measurement
  ...
  gtsam::Pose2 origin(0, 0, 0);
  gtsam::Point2 l1_loc(1, 0);
  gtsam::Point2 l2_loc(0, 1);
  cv.insert(xi, origin);
  cv.insert(l1, l1_loc);
  cv.insert(l2, l2_loc);
  ...
  DCFactorGraph dcfg;
  dcfg.push_back(dcmmf);

  // Make a factor graph and add the factor
  gtsam::DiscreteFactorGraph dfg;
  for (auto& it : dcfg) {
    DCDiscreteFactor dcdf_mmf(it);
    dfg.push_back(dcdf_mmf);
  }
  ...
  for (size_t j = 0; j < dfg.size(); j++) {
    ...
    if (dcDiscreteFactor) {
      dcDiscreteFactor->updateContinuous(cv);
      dcDiscreteFactor->updateDiscrete(dv);
    }
  }

  DiscretePriorFactor dpf(lm2_class, semantic_meas);
  dfg.push_back(dpf);

  dfg.print();
  // gtsam::KeyVector keys;
  // keys.push_back(lc1);
  // gtsam::Ordering ordering(keys);
  // auto output = gtsam::EliminateDiscrete(dfg, ordering);

  // Now test the second key
  gtsam::KeyVector keys_debug;
  keys_debug.push_back(lc2);
  gtsam::Ordering ordering_debug(keys_debug);
  gtsam::DiscreteMarginals marginals(dfg);
  gtsam::Vector probs1 = marginals.marginalProbabilities(lm1_class);
  std::cout << "Probs 1" << probs1 << std::endl;
  gtsam::Vector probs2 = marginals.marginalProbabilities(lm2_class);
  std::cout << "Probs 2" << probs2 << std::endl;

  // auto debug_output = gtsam::EliminateDiscrete(dfg, ordering_debug);
}
